...
import { WrappedNFT } from "ERC20_CCIP/WrappedToken.sol";
...
contract Messenger is CCIPReceiver, OwnerIsCreator {
  ...
  constructor(
    address _router,
    address _link,
    address DrunkAddress
  ) CCIPReceiver(_router) {
    s_linkToken = IERC20(_link);
    //创建一个nft对象
    wnft=WrappedNFT(DrunkAddress);
  }
    ...
    // source chain and sender are allowlisted
  {
    ...
    address owner=req.newOwner;
    wnft.mintTokenwithTokenID(owner,tokenID);
