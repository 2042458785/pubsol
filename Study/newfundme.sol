...
contract HelloWorld{
    ...
    function GetInfoStruct(uint256 inid)public view returns (string memory ){
    ...
    //在上面的情况下 如果交易的次数,也就是合约的调用次数越来越多 那么计算次数也会越来越多 费用也会越来越贵
    //所以我们使用mapping这个结构来进行快速查询
    mapping(uint256 mapid => Info oneinfo) infoMapping;
    function SetInfoMapping(uint256 inid,string memory instring) public {
        Info memory oneinfo=Info(inid,instring,msg.sender);
        infoMapping[inid]=oneinfo;
    }
    function GetInfoMapping(uint256 inid) public view returns (string memory rtstring){
        //我们判断是否存在这个id对应的数据的时候 找到了这个map里面的方便查询为空的数据进行查询
        if (infoMapping[inid].addr==address(0x0)){
            return Addinfo(Strbytes);
        }else {
...
//关于合约的工厂模式调用和普通方式调用
//在工厂模式里面 就是先动态生成一个合约再进行数组里面的存储,和取出对应的合约,进行直接的合约中函数调用
//在普通模式里面 我们先进行合约的初始化,然后我们通过地址对它进行赋值,我们再调用这个合约里面的函数
